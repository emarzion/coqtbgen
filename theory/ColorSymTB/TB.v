...
Require Import TBGen.ColorSymTB.ColorSym.
...
Record TB : Type := {
  ...
  (* just white positions *)
  positions : M (Player * nat);

  (* ditto *)
  last_positions : list (GameState G);
  }.

Definition mirror_res (o : option (Player * nat)) :=
  option_map (fun '(p,n) => (opp p, n)) o.

Definition tb_lookup (tb : TB) (s : GameState G) : option (Player * nat) :=
  match to_play s with
  | White => hash_lookup s (positions tb)
  | Black => mirror_res (hash_lookup s (positions tb))
  end.

(*
Definition last_white_positions (tb : TB) :=
  last_positions tb.

Definition last_black_positions (tb : TB) :=
  map mirror (last_positions tb).
*)
...
Definition eloise_step (tb : TB) : list (GameState G) :=
  let prev := last_positions tb in
  let mirrored_prev := map mirror prev in
  let m :=
    add_positions
      (positions tb)
      (step_player (last_step tb) White)
      (curr tb)
      (last_positions tb) in
  let candidates := concat (map enum_preds mirrored_prev) in
  let candidates' :=
    filter_Nones (fun s => hash_lookup s m) candidates in
  nodup IntHash_dec candidates'.

Definition abelard_step (tb : TB) : list (GameState G) :=
  let prev := last_positions tb in
  let mirrored_prev := map mirror prev in
  let mp :=
    add_positions
  ...
  let candidates' :=
    filter_Nones (fun s => hash_lookup s mp) candidates in
  let candidates'' := filter
    (fun s => forallb (fun m =>
      match hash_lookup (exec_move s m) mp with
      | Some (pl',_) => player_eqb Black pl'
      | None => false
...
Definition TB_init : TB := {|
  ...
  positions := empty;
  last_positions := nodup IntHash_dec (enum_wins Black);
  |}.
...
Definition TB_step (tb : TB) : TB := {|
  ...
  last_positions :=
    ...
    | Eloise => abelard_step tb
    | Abelard => eloise_step tb
    end;
  |}.
...
Proof.
  ...
  pose proof (hash_size_adds_le
    (map (tag (step_player last_step0 White) curr0) last_positions0)
    positions0).
  lia.
Qed.
...
Record TB_valid (tb : TB) : Type := {
  ...
  tb_white : forall {s pl n},
    hash_lookup s (positions tb) = Some (pl, n) ->
    to_play s = White;
  ...
  mate_lp : forall {s pl},
    to_play s = White ->
    mate pl s (curr tb) ->
    In s (last_positions tb);

  lp_mate : forall {s},
    In s (last_positions tb) ->
    mate (step_player (last_step tb) White) s (curr tb);

  lp_NoDup : NoDup (last_positions tb);

  lp_disj : forall s, In s (last_positions tb) -> hash_lookup s (positions tb) = None;

  lp_white : forall s, In s (last_positions tb) -> to_play s = White;
  }.
