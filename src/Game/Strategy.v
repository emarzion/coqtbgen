...
Arguments eloise_strategy {_} {_} {_} _ _.
Arguments abelard_strategy {_} {_} {_} _ _.

Definition strat_id {G} {p} {s : GameState G} (str : strategy p s) : strategy p s :=
  match str with
  | atom_strategy res_pf => atom_strategy res_pf
  | eloise_strategy play_pf m str => eloise_strategy play_pf m str
  | abelard_strategy play_pf strs => abelard_strategy play_pf strs
  end.
...
Inductive winning_strategy {G : Game} (p : Player) : forall {s : GameState G},
  ...
  | eloise_strat_win : forall s (play_pf : to_play s = p) (m : Move s)
      (str : strategy p (exec_move s m)), winning_strategy p str ->
      winning_strategy p (eloise_strategy play_pf m str)
  | abelard_strat_win : forall s (play_pf : to_play s = opp p)
      (strs : forall m, strategy p (exec_move s m)),
      (forall m, winning_strategy p (strs m)) ->
      winning_strategy p (abelard_strategy play_pf strs).
...
CoInductive drawing_strategy {G : Game} (p : Player) : forall {s : GameState G},
  ...
  | eloise_strat_draw : forall s (play_pf : to_play s = p) (m : Move s)
      (str : strategy p (exec_move s m)), drawing_strategy p str ->
      drawing_strategy p (eloise_strategy play_pf m str)
  | abelard_strat_draw : forall s (play_pf : to_play s = opp p)
      (strs : forall m, strategy p (exec_move s m)),
      (forall m, drawing_strategy p (strs m) \/ winning_strategy p (strs m)) ->
      drawing_strategy p (abelard_strategy play_pf strs).