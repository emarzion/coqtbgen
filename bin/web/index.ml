...
module Model = struct
  ...
  let init = Extract.init_RW_State
...
module View = struct
  ...
  let pure_curr_res mp s =
    let text = (
      match Extract.query_RW_TB mp s with
      | Some (pl, n) -> (
        match pl with
  ...
  let pure_move_links mp (rs,rf) s =
    ...
    let move_tups = List.map (fun m ->
      let s' = Extract.coq_RW_exec_move s m in
      (m, Extract.query_RW_TB mp s')
    ) moves in
    ...
    List.concat_map (fun (m,o) ->
      ...
      let text = Extract.print_RW_move s m ^ ": " ^ str in
